...
namespace Mosa.External.x86.Driver
{
    ...
    public unsafe class PCNETII : Network
    {
        ...
        private Buffer* RxBuffers;                   // Physical address of actual receive buffers (< 4 GiB)
        private Buffer* TxBuffers;                   // Physical address of actual transmit buffers (< 4 GiB)
        ...
        public override void Initialize()
        {
            ...
            RxBuffers = (Buffer*)GC.AllocateObject((uint)RxBufferCount);
            TxBuffers = (Buffer*)GC.AllocateObject((uint)TxBufferCount);
            ...
            WriteCSR32(1, address >> 16);
            WriteCSR32(2, address << 16);
        ...
        public override void OnInterrupt()
        {
            while (DriverOwns(Rdes, RxBufferPtr))
            {
                ...
                Buffer* pbuf = (Buffer*)((uint)RxBuffers + RxBufferPtr * NetworkManager.BufferSize);

                // Handle the packet
                NetworkManager.AwaitingBuffers.Add(*pbuf);
        ...
        public override bool SendPacket(Buffer* packet, uint len)
        {
