    ...
    public class MemoryStream : Stream       /* http://msdn.microsoft.com/en-us/library/system.io.stream.aspx */
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                ...
                ASM.MEMCPY((uint)(Intrinsic.GetObjectAddress(buffer) + offset),
                        (uint)(Intrinsic.GetObjectAddress(block) + (int)blockOffset),
                        (uint)copysize);
                lcount -= copysize;
                offset += (int)copysize;
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            try
            {
                do
                {
                    ...
                    ASM.MEMCPY((uint)(Intrinsic.GetObjectAddress(block) + (int)blockOffset),
                        (uint)(Intrinsic.GetObjectAddress(buffer) + offset),
                        (uint)copysize);
                    count -= copysize;
                    offset += copysize;
