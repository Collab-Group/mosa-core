...
namespace System.Collections
{
	...
	public class Queue : ICollection, ICloneable
	{
		...
		private static void ArrayClear<Type>(Type[] array, int start, int end)
		{
			...
			if (end > array.Length) throw new ArgumentOutOfRangeException(nameof(end));
			for (int i = start; i < end; i++) array[i] = default!;
		}
		...
		public virtual void Clear()
		{
			...
			{
				if (_head < _tail)
					ArrayClear(_array, _head, _size);
				else
				{
					ArrayClear(_array, _head, _array.Length - _head);
					ArrayClear(_array, 0, _tail);
				}
	...
	// buffer, so Enqueue can be O(n).  Dequeue is O(1).
	[Serializable]
	public class Queue<T> : ICollection, ICloneable
	{
		private T[] _array; // Do not rename (binary serialization)
		private int _head; // First valid element in the queue. Do not rename (binary serialization)
		private int _tail; // Last valid element in the queue. Do not rename (binary serialization)
		private int _size; // Number of elements. Do not rename (binary serialization)
		private readonly int _growFactor; // 100 == 1.0, 130 == 1.3, 200 == 2.0. Do not rename (binary serialization)
		...
		// Implements a synchronization wrapper around a queue.
		private sealed class SynchronizedQueue : Queue<T>
		{
			...
			public override object Clone()
			{
				lock (_root)
				{
					return new SynchronizedQueue((Queue<T>)_q.Clone());
				}
			}
		...
		private sealed class QueueEnumerator : IEnumerator, ICloneable
		{
			...
			internal QueueEnumerator(Queue<T> q)
			{
				_q = q;
				_version = _q._version;
				_index = 0;
